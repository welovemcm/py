...
from Car import Car
from CarGeneratorMultiTypes import CarGeneratorMultiTypes
from Map import Map
import random
import pickle
...
def one_run(traffic_flow):
    ...
    while (case < 10):
        debug = False
        time_t = 0
        mmap = Map('./map/lr2m_1ci1dao.testmap')
        if (debug):
            print mmap.get_L()
            print mmap.get_B()
            print mmap.get_length()

        car_generator = CarGeneratorMultiTypes.init_with_default_paras(mmap, traffic_flow)
...
def vary_traffic_flow():
    ...
    with open('./exp/exp_mixed_tollbooth_types_etc_6atc_mtc.dump', 'wb') as f:
