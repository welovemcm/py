...
def one_run(traffic_flow):
    ...
    while (case < 10):
        debug = False
        time_t = 0
        mmap = Map('./map/lr2m_1ci1dao_short.testmap')
        if (debug):
            print mmap.get_L()
            print mmap.get_B()
            print mmap.get_length()

        car_generator = CarGenerator(mmap, traffic_flow, car_is_auto=True)  # 每秒10辆车
...
def vary_traffic_flow():
    ...
    with open('./exp/lr2m_1ci1dao_short_a.dump', 'wb') as f:
